...
// v2 feed layer (services/scraper/feeds) — mirrors
// scripts/run_feed.py::PROVIDERS / feed_scheduler.py's FEED_TIER_SOURCES
// (which derives its source list from that same dict). Keep in sync
// whenever a new provider ships.
const SOURCES = [
  { key: "greenhouse", label: "Greenhouse", cadence: "Daily" },
  { key: "lever", label: "Lever", cadence: "Daily" },
  { key: "ashby", label: "Ashby", cadence: "Daily" },
...
export function FeedSchedulerProgress() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const res = await adminFetch("/api/feeds/scheduler/progress", { cache: "no-store" });
  ...
  async function handleTrigger(source: string) {
    ...
    try {
      const res = await adminFetch(`/api/feeds/scheduler/trigger/${source}`, { method: "POST" });
      const data = await res.json();
      if (!data.started) {
        showToast(data.reason ?? "Could not start this sweep.", "error");
      } else {
        showToast(`Feed poll started for ${source}.`, "success");
      }
    } catch {
            ...
            >
              ...
              {isWaiting && (
                <div style={{ marginTop: 8, fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--ink-muted)" }}>
                  {runningSourceLabel} is running right now — only one feed provider polls at a
              ...
              {isRunning && current ? (
                ...
                !isWaiting && (
                  <div style={{ marginTop: 10, fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--ink-faint)" }}>
                    {lastRun ? (
                      <span>
                        Last run: {formatDateTime(lastRun.finished_at)} · {lastRun.saved_count} posting
